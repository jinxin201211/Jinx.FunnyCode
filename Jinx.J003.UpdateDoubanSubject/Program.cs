...
namespace Jinx.J003.UpdateDoubanSubject
{
    class Program
    {
        ...
        private static void GetCelebIDsMovieID()
        {
            using (MovieWarehouseEntities _db = new MovieWarehouseEntities())
            {
                foreach (var item in _db.Douban_Celebrity_ID.Where(p => p.ReadMovie == false))
                {
        ...
        private static void GetMovieIDsFromCelebID()
        {
            using (MovieWarehouseEntities _db = new MovieWarehouseEntities())
            {
                foreach (var item in _db.Douban_Celebrity_ID.Where(p => p.Split == false))
                {
                    if (string.IsNullOrWhiteSpace(item.Movie))
                        continue;
                    ...
                    foreach (var id in movies)
                    {
                        ...
                        {
                            _db2.Douban_Movie_ID.Add(new Douban_Movie_ID { ID = sid, Read = false, FZF = false, Split = false });
                            Console.WriteLine("Add ID: " + id);
                        }
                    }
                    _db2.Douban_Celebrity_ID.FirstOrDefault(p => p.ID == item.ID).Split = true;
                    _db2.SaveChanges();
                }
            }
        }
        ...
        {
            MovieWarehouseEntities _db = new MovieWarehouseEntities();
            List<Douban_Movie_ID> allmovie = _db.Douban_Movie_ID.Where(p => p.Split == false).ToList();
            Console.WriteLine("总共电影" + allmovie.Count + "部");
            int count = 0;
            int k = 0;
            List<Douban_Celebrity_ID> celebIDList = new List<Douban_Celebrity_ID>();
            foreach (var item in allmovie)
            {
                    ...
                    if (!string.IsNullOrEmpty(listMovieCelebStr[i]))
                    {
                        ...
                        for (int j = 0; j < listCeleb.Count; j++)
                        {
                            if (!string.IsNullOrEmpty(listCeleb[j].ID))
                            {
                                ...
                                if (_db.Douban_Celebrity_ID.Where(p => p.ID == number).Count() == 0 && celebIDList.Where(p => p.ID == number).Count() == 0)
                                {
                                    Douban_Celebrity_ID model = new Douban_Celebrity_ID();
                                    model.ID = number;
                                    model.Read = false;
                                    model.FZF = false;
                                    model.ReadMovie = false;
                                    model.Split = false;
                                    celebIDList.Add(model);
                                    SWLog.WriteLine($"[INFO][MESSAGE][{DateTime.Now}][{TaskName}][New Celeb ID:{model.ID}]");
                                    SWLog.Flush();
                ...
                {
                    _db.Douban_Celebrity_ID.AddRange(celebIDList);
                    _db.SaveChanges();
                    celebIDList.Clear();
                    Console.WriteLine("1000 Succeed, Save Changes");
                }
                k++;
        ...
        //重置影人读取进度
        private static void ResetCelebIDAllNoReadSplit()
        {
            Stopwatch sw = new Stopwatch();
            sw.Start();
            MovieWarehouseEntities _db = new MovieWarehouseEntities();
        ...
        public static string GetCelebrityMoviesNew(string celeb)
        {
            ...
            while (!string.IsNullOrEmpty(url))
            {
                ...
                foreach (var item in linkNodes)
                {
                    ...
                    if (mc_date != null)
                    {
                        if (!string.IsNullOrEmpty(mc_date.InnerText))
                        {
                            int iparse = 0;
                            if (int.TryParse(mc_date.InnerText, out iparse))
                            {
        ...
        //更新的电影ID表的影人信息
        public static void UpdateMovieIDsCeleb()
        {
            string connString = System.Configuration.ConfigurationManager.AppSettings["DatabaseConnStr"].ToString();
            using (SqlConnection conn = new SqlConnection(connString))
            {
                conn.Open();
                SqlCommand command = new SqlCommand(@"UPDATE A
  SET A.Directors = B.Directors,A.Writers=B.Writers,A.Casts=B.Casts
  FROM [MovieWarehouse].[dbo].[Douban_Movie_ID] A, [MovieWarehouse].[dbo].[Douban_Movie_Info] B
  WHERE A.ID=B.ID", conn);
                int count = command.ExecuteNonQuery();
                Console.WriteLine("更新的电影ID表的影人信息行数：" + count);
                command.Dispose();

                SqlCommand command2 = new SqlCommand(@"UPDATE [Douban_Movie_ID] SET [Split]=1 WHERE [Directors] IS NULL AND [Writers] IS NULL AND [Casts] IS NULL", conn);
                count = command2.ExecuteNonQuery();
                Console.WriteLine("没有影人的电影行数：" + count);
                command2.Dispose();
        ...
        public static void GetAllPeopleContact()
        {
            int Round = 1;
            while (true)
            {
                Console.WriteLine("Round:\t" + Round++);
                using (MovieWarehouseEntities database = new MovieWarehouseEntities())
                {
                    var allPeopleNotContacted = database.Douban_People_ID.Where(p => !p.Contacted && !p.FZF);
                    foreach (var item in allPeopleNotContacted)
                    {
                        MovieWarehouseEntities _db2 = new MovieWarehouseEntities();
                        Douban_People_ID people = _db2.Douban_People_ID.Where(p => p.ID == item.ID).First();
                        try
                        {
                            GetPeopleContact(item.ID);
                            Console.WriteLine("成功：" + item.ID);
                        }
                        catch (Exception e)
                        {
                            people.FZF = true;
        ...
        public static void GetPeopleContact(string peopleid)
        {
            string urlHead = string.Format("https://www.douban.com/people/{0}/contacts", peopleid);
            string url = string.Format("https://www.douban.com/people/{0}/contacts", peopleid);
            ...
            if (!string.IsNullOrWhiteSpace(htmlCode))
            {
                ...
                HtmlDocument doc = new HtmlDocument();
                doc.LoadHtml(htmlCode);
                //doc.Load(@"D:\CUIT\C#.NET\GC_GatherCode\UpdateDoubanSubject\6860160.html");

                HtmlNode gridviewNode = doc.DocumentNode.CssSelect("div.article").FirstOrDefault();
                var linkNodes = gridviewNode.CssSelect("dl.obu");
                using (MovieWarehouseEntities database = new MovieWarehouseEntities())
                {
                    foreach (var item in linkNodes)
                    {
                        var dt = item.CssSelect("dt").First();
                        var link = dt.CssSelect("a").First();
                        var href = link.Attributes["href"].Value;
                        string[] hrefs = href.Split('/');
                        string ContactID = hrefs[4];
                        database.Douban_People_Contact.Add(new Douban_People_Contact()
                        {
                            ID = Guid.NewGuid(),
                            PeopleID = peopleid,
                            ContactID = ContactID
        ...
        {
            string ID = url.Split('/').Count() >= 5 ? url.Split('/')[4] : url;//.Replace("https://movie.douban.com/celebrity/", "").Replace("/movies?sortby=time&format=text&", "");
            try
            {
